...
class ProtocolProcessor:
    ...
    def setup_ui(self):
        ...
        drop_frame = ttk.LabelFrame(
            self.root, text="1. Виберіть файл протоколу", padding="20")
        drop_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        self.drop_label = ttk.Label(drop_frame,
                                    text="Натисніть кнопку нижче, щоб вибрати файл .docx",
                                    font=('Arial', 12),
                                    background='#e8f4f8',
                                    relief=tk.RIDGE,
                                    padding=50)
        ...
        result_frame = ttk.LabelFrame(
            self.root, text="2. Результат обробки", padding="10")
    ...
    def select_file(self):
        ...
        if file_path:
            ...
            self.file_info.config(
                text=f"✓ Вибрано: {file_path.split('/')[-1]}")
            self.drop_label.config(
                text=f"Файл завантажено:\n{file_path.split('/')[-1]}")
    ...
    def extract_protocol_data(self, doc_path):
        ...
        protocol_match = re.search(
            r'ПРОТОКОЛ\s*№\s*(\d+)', full_text, re.IGNORECASE)
        ...
        executors = [
            ...
            (r'Сергій БОЧІН', 'Бочін Сергій'),
            (r'Ларисі ГАВРИЛЕНКО', 'Гавриленко Лариса'),
            (r'Лариса ГАВРИЛЕНКО', 'Гавриленко Лариса'),
            (r'Аліні ЛИСЕНКО-ГАМАНЮК', 'Аліна Лисенко-Гаманюк'),
            (r'Аліна ЛИСЕНКО-ГАМАНЮК', 'Аліна Лисенко-Гаманюк'),
            (r'Олексію БУКА', 'Бука Олексій'),
            (r'Олексій БУКА', 'Бука Олексій'),
            (r'Юлії КОРЕНКОВІЙ', 'Коренкова Юлія'),
            (r'Юлія КОРЕНКОВА', 'Коренкова Юлія'),
            (r'Михайлу ЛЕОНТЬЄВУ', 'Леонтьєв Михайло'),
            (r'Михайло ЛЕОНТЬЄВ', 'Леонтьєв Михайло'),
            (r'Андрію ЗАЛУСЬКОМУ', 'Залуський Андрій'),
            (r'Андрій ЗАЛУСЬКИЙ', 'Залуський Андрій'),
            (r'Андрію ІЩЕНКО', 'Іщенко Андрій'),
            (r'Андрій ІЩЕНКО', 'Іщенко Андрій'),
            (r'Ніні МОСКАЛЕНКО', 'Москаленко Ніна'),
            (r'Ніна МОСКАЛЕНКО', 'Москаленко Ніна'),
            (r'Олегу ОЛІНКЕВИЧ', 'Олінкевич Олег'),
            (r'Олег ОЛІНКЕВИЧ', 'Олінкевич Олег'),
            (r'Ірині ДИКУШІ', 'Ірина Дикуша'),
            (r'Ірині БАРДАКОВІЙ', 'Ірина Бардакова'),
            (r'Ірина БАРДАКОВА', 'Ірина Бардакова'),
            (r'Ірині БОРДАКОВІЙ', 'Ірина Бордакова'),
            (r'Ірина БОРДАКОВА', 'Ірина Бордакова'),
        ...
        for i, line in enumerate(lines):
            ...
            if is_new_decision:
                ...
                if executor_pattern_matched:
                    clean_line = re.sub(
                        executor_pattern_matched, '', clean_line, flags=re.IGNORECASE).strip()

                # Перша літера з великої
                if clean_line:
                    clean_line = clean_line[0].upper(
                    ) + clean_line[1:] if len(clean_line) > 1 else clean_line.upper()
    ...
    def sort_data(self, data):
        ...
        def sort_key(item):
            ...
            is_meeting_item = 'наступне засідання' in item['text'].lower(
            ) or 'призначити наступне' in item['text'].lower()
    ...
    def update_display(self):
        ...
        self.result_text.insert(
            tk.END, f"Знайдено доручень: {len(sorted_data)}\n")
        self.result_text.insert(
            tk.END, f"Сортування: за датою, потім за виконавцем\n\n")
        ...
        self.result_text.tag_config(
            'header', foreground='blue', font=('Arial', 10, 'bold'))
    ...
    def process_file(self):
        ...
        try:
            ...
            self.update_display()
            messagebox.showinfo(
                "Успіх", f"Оброблено {len(self.output_data)} доручень!")
    ...
    def create_table_document(self, output_path):
        ...
        run2 = header2.add_run(
            "з виконання протокольних доручень голови районної адміністрації Ільченко С.В.")
